...
import { registerMachineHandlers } from '../modules/machineHandlers/registerMachineHandlers';
...
export class ApiMachineClient {
    ...
    constructor(
        ...
        registerMachineHandlers(this.rpcHandlerManager);
    }
    ...
    setRPCHandlers({
        ...
        this.rpcHandlerManager.registerHandler('spawn-happy-session', async (params: any) => {
            const { directory, sessionId, machineId, approvedNewDirectoryCreation, agent, permissionMode, modelMode, effortLevel, environmentVariables, token, resumeClaudeSessionId, resumeCodexThreadId, resumeGrokSessionId, parentSessionId, forkedFromMessageId, isSideChat } = params || {};
            ...
            const result = await spawnSession({ directory, sessionId, machineId, approvedNewDirectoryCreation, agent, permissionMode, modelMode, effortLevel, environmentVariables, token, resumeClaudeSessionId, resumeCodexThreadId, resumeGrokSessionId, parentSessionId, forkedFromMessageId, isSideChat });
    ...
    private sendKeepAlive() {
        ...
        const cliAvailabilityChanged = !prev || prev.claude !== newAvailability.claude || prev.codex !== newAvailability.codex || prev.gemini !== newAvailability.gemini || prev.grok !== newAvailability.grok || prev.openclaw !== newAvailability.openclaw || prev.agy !== newAvailability.agy;
